// This is a GUI-only copy of the root Deck.cpp with one fix: the RNG is no
// longer reseeded inside shuffle(). It is seeded ONCE in main.cpp instead.
//
// Why: the original shuffle() called srand(time(0)) every time it ran.
// Two shuffles within the same second (very easy to trigger in the GUI,
// where rounds happen fast) produced the exact same deck.
//
// The console version's Deck.cpp is untouched - this file only affects
// the graphical build.
...
void Deck::shuffle() {
    // The RNG is seeded ONCE in main.cpp. Calling srand(time(0)) inside
    // shuffle() means two shuffles in the same second produce the exact
    // same deck, so the seeding was moved out of here.
